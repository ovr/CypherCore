...
namespace Framework.Cryptography
{
    ...
    public partial class SRP6
	{
        ...
        public SRP6(string username, BigInteger credentials, bool isServer)
        ...
        public SRP6(string username, BigInteger verifier, BigInteger salt)
            ...
            get
            {
                if (m_sessionKey == null)
                {
                    ...
                    if (IsServer)
                    {
                        ...
                        var b = a * PublicEphemeralValueA % Modulus;
                        S = b.ModPow(m_secretEphemeralValueB, Modulus);
                    }
                    else
                    {
                        var modulus = Parameters.Modulus;
                        var a = Parameters.Generator.ModPow(CredentialsHash, modulus);
                        var b = (Multiplier * a) % modulus;
                        var c = (PublicEphemeralValueB - b) % modulus;
                        var d = m_secretEphemeralValueA + ScramblingParameter * CredentialsHash;
